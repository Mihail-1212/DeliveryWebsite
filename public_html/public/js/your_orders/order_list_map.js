window.addEventListener("load", async function(){
    ...
    var orderCancle_show = document.getElementById('orderCancle');
    //
    ...
    var modal_window_cancel_order = document.getElementById('modal_window_cancel_order');
    // modal_window_cancel_order
    ...
    var cancel_order = new Modal_Window(orderCancle_show, modal_window_cancel_order);
    
    var cancelOrderButton = document.getElementById('orderCancle');
    
    async function FetchGetOrder(parameters){
        let response = await fetch( url + 'your_orders//getOrderInfo', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            var date_exec = Date.parse(orderInfo['execution_date']);
            if(!isNaN(date_exec)){
                date_exec = await getDateTime(date_exec);
            } else {
                date_exec = '-';
            }
            document.getElementById('info_body_create_date').innerHTML = date;
            document.getElementById('info_body_execution_date').innerHTML = date_exec;
            document.getElementById('info_body_status').innerHTML = orderInfo['status']['short_text'];
            ...
            if(orderInfo['status']['status'] == 'accepted'){
                cancelOrderButton.style.visibility = "visible";
            } else {
                cancelOrderButton.style.visibility = "collapse";
            }
            //basket_block
